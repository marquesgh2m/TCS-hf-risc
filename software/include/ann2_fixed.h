...
struct f_neural_net_weights_s {
	fixed_t hidden_weights[(INPUT_NEURONS+1) * HIDDEN_NEURONS];
	fixed_t output_weights[(HIDDEN_NEURONS+1) * OUTPUT_NEURONS];
};

struct f_neural_net_s {
	fixed_t input[N_PATTERNS * INPUT_NEURONS];
	fixed_t target[N_PATTERNS * OUTPUT_NEURONS];
	fixed_t hidden[HIDDEN_NEURONS];
	fixed_t output[OUTPUT_NEURONS];
	struct f_neural_net_weights_s weights;
};

// fixed functions
fixed_t _sigmoid(fixed_t val);
void _load(struct f_neural_net_s *net, fixed_t *input, fixed_t *target);
void _infer(struct f_neural_net_s *net, int p);
void _show_training(struct f_neural_net_s *net);
void _train(struct f_neural_net_s *net);
